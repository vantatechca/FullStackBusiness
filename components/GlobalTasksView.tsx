...
// Column widths — fixed so long text wraps instead of pushing columns
const COL_WIDTHS = {
  department: '160px',
  task:       '220px',
  recurrence: '100px',
...
// Local-state assignee select — selection appears instantly
  ...
  return (
    <select
      ...
      onChange={e => {
        ...
        setLocal(val);   // instant
        onChange(val);   // optimistic update up
      }}
      className={`w-full px-2 py-1 text-sm rounded-md border transition-colors outline-none focus:ring-1 focus:ring-blue-300 cursor-pointer ${
        isMismatch
    ...
    >
      <option value="">— Unassigned —</option>
      {isMismatch && (
        <option value={local}>{local} ⚠️</option>
      )}
      {allMemberNames.map(name => (
        <option key={name} value={name}>{name}</option>
      ))}
...
export default function GlobalTasksView() {
  ...
  // Optimistic — state first, API in background
      ...
      {filtered.length === 0 ? (
        ...
        <div className="overflow-x-auto border border-gray-200 rounded-lg">
          <table className="w-full text-sm table-fixed">
            <thead>
              <tr className="border-b-2 border-gray-200">
                <th style={{ width: COL_WIDTHS.department }} className="text-left text-[11px] font-bold text-[#475569] uppercase tracking-wider px-3 py-2.5 bg-gray-50/80">Department</th>
                <th style={{ width: COL_WIDTHS.task }}       className="text-left text-[11px] font-bold text-[#475569] uppercase tracking-wider px-3 py-2.5 bg-gray-50/80">Task</th>
                <th style={{ width: COL_WIDTHS.recurrence }} className="text-left text-[11px] font-bold text-[#475569] uppercase tracking-wider px-3 py-2.5 bg-gray-50/80">Recurrence</th>
                <th style={{ width: COL_WIDTHS.status }}     className="text-left text-[11px] font-bold text-[#475569] uppercase tracking-wider px-3 py-2.5 bg-gray-50/80">Status</th>
                <th style={{ width: COL_WIDTHS.assignee }}   className="text-left text-[11px] font-bold text-[#475569] uppercase tracking-wider px-3 py-2.5 bg-gray-50/80">Assignee</th>
                <th style={{ width: COL_WIDTHS.deadline }}   className="text-left text-[11px] font-bold text-[#475569] uppercase tracking-wider px-3 py-2.5 bg-gray-50/80">Deadline</th>
                <th style={{ width: COL_WIDTHS.priority }}   className="text-left text-[11px] font-bold text-[#475569] uppercase tracking-wider px-3 py-2.5 bg-gray-50/80">Priority</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((task, idx) => (
                <tr
                  key={task.id}
                  className={`border-b border-gray-100 align-top ${idx % 2 === 1 ? 'bg-[#fafbfc]' : 'bg-white'}`}
                >
                  {/* Department — wraps within fixed column width */}
                  <td className="px-3 py-2 text-gray-700">
                    <span className="block break-words whitespace-normal leading-snug">{task.dept_name}</span>
                  </td>

                  {/* Task — wraps within fixed column width */}
                  ...
                  {/* Assignee — local state select, instant update */}
                  <td className="px-3 py-1.5">
                    <AssigneeSelect
                      value={task.assignee || ''}
                  ...
                  {/* Deadline */}
                  <td className="px-3 py-2 text-gray-600 whitespace-nowrap text-xs">{task.deadline}</td>

                  {/* Priority */}
                  <td className={`px-3 py-2 text-xs whitespace-nowrap ${priorityColor(task.priority)}`}>
                    {task.priority}
                  </td>
                </tr>
              ))}
