...
export default function QuickAddButton() {
  ...
  return (
    <>
      <div className="fixed bottom-6 right-6 z-40 flex flex-col-reverse items-end gap-2">
          ...
          <div className="flex flex-col-reverse gap-2 mb-1">
            {actions.map((action, i) => (
              <button
                ...
                style={{ animation: `fab-pop-in 200ms cubic-bezier(0.34, 1.56, 0.64, 1) ${i * 50}ms both` }}
              >
                {action.icon}
                {action.label}
              </button>
        ...
        <button
          ...
          className={`w-14 h-14 rounded-2xl shadow-lg flex items-center justify-center transition-all duration-200 ${
            expanded ? 'bg-gray-700 hover:bg-gray-800 rotate-45' : 'bg-[#3b82f6] hover:bg-[#2563eb]'
          }`}
        >
      ...
      <QuickAddModal open={modalOpen} onClose={() => setModalOpen(false)} entityType={modalType} />
