...
export default function GMBView() {
  ...
  const handleAdd = useCallback(async () => {
    ...
    const optimistic = {
      id: tempId, name: '', address: '', status: 'Pending',
      rating: 0, reviews: 0, notes: '', created_at: '',
    } as GMBListing;
    ...
    const res = await fetch('/api/gmb-listings', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: '', address: '', status: 'Pending', rating: 0, reviews: 0, notes: '' }),
    });
